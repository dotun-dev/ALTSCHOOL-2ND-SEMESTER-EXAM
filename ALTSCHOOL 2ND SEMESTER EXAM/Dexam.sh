...
sudo chmod -R 755 /var/www/html/app\" > Dexam.sh"

echo "
in_slave_ip=\$(ip addr show enp0s8 | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | tr -d '\r')" >> Dexam.sh
...
cat Dexam.sh | vagrant ssh Master -c "cat >> Dexam.sh"
...
sudo service apache2 restart\" >> Dexam.sh"

vagrant ssh Master -c "chmod 777 Dexam.sh"
  ...
  tasks:
    - name: Copy the Bash script to the slave machine
      copy:
        ...
        dest: ~/Dexam.sh  # Replace with the desired path on the slave
